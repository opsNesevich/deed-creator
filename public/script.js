  ...
  try {
    ...
    const claudeRes = await fetch('/api/claude', {
      ...
      body: JSON.stringify({
        ...
        max_tokens: 2048,
        system: 'You are a NJ deed data extraction specialist for Nesevich Law. CRITICAL RULES: 1) Read ONLY from the COVER SHEET - never from chain-of-title body entries. 2) The Second Party / Grantee on the cover sheet is the CURRENT OWNER and becomes the new GRANTOR in the documents you populate - put their name in "grantor" (and "grantor2" if there are two owners). 3) Municipality format: always "[Name] Township" or "[Name] Borough" - NEVER "Township of [Name]". 4) propCity is the mailing city shown in the property address (e.g. "Marlton") - NOT the municipality name. 5) grantorAddr must be comma-separated: "Street, City, ST ZIP" (e.g. "123 Oak Ave, Moorestown, NJ 08057"). 6) All dates must be in YYYY-MM-DD format (e.g. 2019-06-15). 7) priorCounty is ONLY the county name (e.g. "Burlington") - no extra words. 8) Return ONLY valid JSON with no markdown, no explanation.',
        messages: [{
          role: 'user',
          content: images.map(function(img) { return { type: 'image', source: { type: 'base64', media_type: 'image/jpeg', data: img } }; }).concat([{
            type: 'text',
            text: 'Extract all fields from this NJ deed cover sheet. Return ONLY valid JSON (no markdown, no extra text):\n{"grantor":"Second Party/Grantee full name (becomes new grantor)","grantor2":"Second co-owner name or empty string","relationship":"e.g. husband and wife, or empty","grantorAddr":"Street, City, ST ZIP (comma-separated)","grantorZip":"5-digit ZIP of grantor address","municipality":"e.g. Moorestown Township","county":"county name only e.g. Burlington","block":"tax map block number","lot":"tax map lot number","qualifier":"lot qualifier or empty","propAddr":"full property street address","propCity":"mailing city e.g. Marlton","propZip":"property ZIP code","priorDeedDate":"YYYY-MM-DD","priorRecordedDate":"YYYY-MM-DD","priorCounty":"county name only where prior deed was recorded e.g. Burlington","priorBook":"deed book number","priorPage":"page number","instrumentNo":"instrument number or empty","legalDescription":"full legal description text"}'
          }])
        }]
      })
    });
